...
public class ConfigService {
   ...
   */
  public List<ConfigDto> getAllConfig(Integer id) {
    Optional<List<Config>> configList = configRepo.findAllByUserId(id);
    List<ConfigDto> configDtoList = new ArrayList<>();
    if (configList.isPresent()) {
      for (Config config : configList.get()) {
   ...
   * @return the modified config.
   */
  public ConfigDto handleConfigValueWithUser(ConfigDto config, Integer id) {

    Optional<Config> configFound = configRepo.getConfigByConfigName_NameAndUserId(config.getConfigName(), id);
    if (configFound.isPresent()) {
      configFound.get().setConfigValue(config.getConfigValue());
      configRepo.save(configFound.get());
    } else {
