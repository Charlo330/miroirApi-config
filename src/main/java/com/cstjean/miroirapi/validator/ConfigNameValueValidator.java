  ...
  public boolean isValid(ConfigDto configDto,
    ...
    try {
      return switch (configDto.getConfigName()) {
        ...
        case "cityWeather" -> configDto.getConfigValue().matches("^[A-Za-z\\s\\-']+$");

        case "tempMax", "tempMin" -> configDto.getConfigValue().matches("^[0-5]?[0-9]$");

        case "humidityMax", "humidityMin" -> configDto.getConfigValue().matches("^[0-9]?[0-9]$");
